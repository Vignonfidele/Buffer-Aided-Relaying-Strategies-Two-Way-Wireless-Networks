function[OUTC1]=Funtion_Com_dois_Buffer_no_relay_EF(n1,n2,n3,ptdb,P,Max,N)
    OUTC1=[];
    pt=10.^(ptdb/10);
    No=1; R=1; alpha=4; 
for d=n1:n3:n2  
    ...
    i=i+1; t=t+1;                                                                                               %(incremento de 1 a i e t para inicializar con el primero elemento del vector Paquete e del vector de la informacion mutua.
           if (ImAr(1,t)>=R && ImBr(1,t)<R)                                                                         %A-->R OK and B---> Not Ok  
                ...
                elseif AR==Max                                                                                      % el Buffer AR esta lleno
                    ...
                    PacksB(i)=3;  fB=fB+1;
                    if BR>0
                       PacksB(ContB(1,1:min(3,BR)))=4; fB=fB+min(3,BR); ContB(1,1:min(3,BR))=0;  ContB(ContB==0)=[]; BR=BR-min(3,BR);                                           %(Hay un elemento en BR, el numero de este paquete fue almacenado en la primera posicion de ContB, Este numero se extrae, asigno el valor 4, inclemento fB, zero ContB, decremento BR de 1, )
                    end
                end
                 %A-->R Not OK and B--->  Ok 
            elseif (ImAr(1,t)<R && ImBr(1,t)>=R)                                                                    %Sistema simetro, aqui executo las misma tareas que fueron, %excecutada en el A-->R OK and B---> Not Ok.
               ...
               elseif BR==Max
                    ...
                    PacksB(i)=3;  fB=fB+1;
                     if AR>0
                        PacksA(ContA(1,1:min(3,AR)))=4; fA=fA+min(3,AR); ContA(1,1:min(3,AR))=0; ContA(ContA==0)=[]; AR=AR-min(3,AR);  
                     end 
               end 
