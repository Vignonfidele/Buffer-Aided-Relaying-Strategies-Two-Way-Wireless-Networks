function[OUTA1,OUTB1]=Funtion_Com_dois_Buffer_no_relay(ptdb,Max)
    OUTA1=[];
    OUTB1=[];
    OutageA=0; OutageB=0;
    R=1;  No=1;
...
for P=700000:100000:1000000
                                                   %(Numero de paquete a ser simulado + un margen de error )
     P=P+Error;
        ...
        i=i+1; t=t+1;                                                                                               %(incremento de 1 a i e t para inicializar con el primero elemento del vector Paquete e del vector de la informacion mutua.
           if (ImAr(1,t)>=R && ImBr(1,t)<R)                                                                         %A-->R OK and B---> Not Ok  
                ...
                elseif AR==Max                                                                                      % el Buffer AR esta lleno
                    ...
                    PacksB(i)=3;  fB=fB+1;
                    if BR>0
                       PacksB(ContB(1,1:min(BR,3)))=4; fB=fB+min(BR,3); ContB(1,1:min(BR,3))=0;  ContB(ContB==0)=[]; BR=BR-min(BR,3);                                           %(Hay un elemento en BR, el numero de este paquete fue almacenado en la primera posicion de ContB, Este numero se extrae, asigno el valor 4, inclemento fB, zero ContB, decremento BR de 1, )
                    end
                end
                 %A-->R Not OK and B--->  Ok 
            elseif (ImAr(1,t)<R && ImBr(1,t)>=R)                                                                    %Sistema simetro, aqui executo las misma tareas que fueron, %excecutada en el A-->R OK and B---> Not Ok.
               ...
               elseif BR==Max
                    ...
                    PacksB(i)=3;  fB=fB+1;
                     if AR>0
                        PacksA(ContA(1,1:min(AR,3)))=4; fA=fA+min(AR,3); ContA(1,1:min(AR,3))=0;  ContA(ContA==0)=[]; AR=AR-min(AR,3);
                     end 
               end 
