function[OUTA3, OUTB3]=Funtion_Com_Buffer_no_relay_Fonte1_otima(n1,n2,P)
    OutageA=0; OutageB=0;
    OUTA3=[]; OUTB3=[];
for ptdb=n1:n2 
    R=1;
    No=1; d=0.416; d1=d; d2=1-d; N=2e6;  alpha=4 ; 
    ...
    ImAr=(1/3)*log2(1+SNRAr);    ImBr=(1/3)*log2(1+SNRBr); Error=20;  
                ...
                elseif A>=3
                    ...
                    elseif AR<=Max-3
                        if BR==0 
                          ...
                          PacksA(ContA(1,1:2))=2;  ContAR=[ContAR ContA(1,1:2)]; AR=AR+1; ContA(1,1:2)=0; ContA(ContA==0)=[]; ContAR(ContAR==0)=[];
                          PacksA(i)=1; ContA(A-1)=i; A=A-1; 
                          PacksB(ContBR(1))=4;  fB=fB+1; ContBR(1)=0; ContBR(ContBR==0)=[]; BR=BR-1;
                        end
                    end
